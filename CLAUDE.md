...
После значительных изменений заливай на хостинг сразу.
...
Деплой — `docs/DEPLOY.md` §7. Там же два правила против лишних минут:
`npm ci` на сервере только когда изменился `package-lock.json`, и собирать
обязательно на сервере — локальный `standalone` тащит нативные бинарники
своей платформы.
