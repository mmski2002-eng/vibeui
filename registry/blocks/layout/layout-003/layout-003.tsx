...
import { Card063 } from "@/registry/components/card/card-063/card-063"
...
[data-vibeui-block="layout-003"][data-rules] [data-vibeui-block="card-063"]{border-top:1px solid var(--vibeui-card-063-edge-hover)}
...
export function Layout003({
  ...
  const tile = (area: (typeof SLOTS)[number], className?: string, key?: string) => (
    <Card063 key={key ?? area} data-part="tile" {...items?.[SLOTS.indexOf(area)]} numbers={numbers} index={SLOTS.indexOf(area)} ghost={GHOST[area]} data-area={area} className={className} accent={accent} />
  )
