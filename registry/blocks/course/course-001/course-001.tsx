...
[data-vibeui-block="course-001"] [data-part="tab"]{position:relative;z-index:1;display:flex;flex-direction:column;justify-content:flex-end;width:100%;height:100%;min-height:inherit;padding:1.25rem;border:0;background:transparent;color:inherit;font:inherit;text-align:left;cursor:pointer}
[data-vibeui-block="course-001"] [data-part="tab"]:focus-visible{outline:2px solid var(--vibeui-course-001-accent);outline-offset:-4px;border-radius:1.25rem}
[data-vibeui-block="course-001"] [data-part="label"]{display:flex;align-items:center;gap:.6rem;font-family:var(--vibeui-course-001-display);font-size:1rem;font-weight:600;line-height:1.2}
[data-vibeui-block="course-001"] [data-part="label"]::before{content:"";width:.6rem;height:.6rem;border-radius:50%;border:2px solid var(--vibeui-course-001-accent);transition:background .3s}
...
[data-vibeui-block="course-001"] [data-part="body"]{position:relative;z-index:1;display:grid;gap:.9rem;padding:0 1.25rem 1.25rem;animation:vibeui-course-001-in .5s .15s cubic-bezier(.2,.8,.2,1) both}
@keyframes vibeui-course-001-in{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:none}}
[data-vibeui-block="course-001"] [data-part="note"]{display:inline-block;justify-self:start;padding:.3rem .65rem;border-radius:.5rem;background:var(--vibeui-course-001-marker);color:#1a2e05;font-family:var(--vibeui-course-001-display);font-size:.72rem;font-weight:600}
[data-vibeui-block="course-001"] [data-part="card-title"]{margin:0;font-family:var(--vibeui-course-001-display);font-size:1.25rem;font-weight:600;line-height:1.2}
[data-vibeui-block="course-001"] [data-part="text"]{margin:0;opacity:.75;max-width:34rem}
[data-vibeui-block="course-001"] [data-part="outcomes-label"]{margin:.25rem 0 0;font-size:.7rem;letter-spacing:.14em;text-transform:uppercase;opacity:.6;font-weight:700}
[data-vibeui-block="course-001"] [data-part="outcomes"]{margin:0;padding:0;list-style:none;display:grid;gap:.5rem}
[data-vibeui-block="course-001"] [data-part="outcomes"] li{display:flex;gap:.65rem;align-items:flex-start;animation:vibeui-course-001-in .45s cubic-bezier(.2,.8,.2,1) both;animation-delay:calc(.25s + var(--vibeui-course-001-n) * 70ms)}
        ...
        <div data-part="shell">
          ...
          <ul data-part="tiles" role="tablist" aria-label="Кто вы сейчас">
            {personas.map((item, index) => {
              ...
              return (
                <li key={item.label} data-part="tile" data-open={open} onMouseEnter={() => setActive(index)}>
                  <span data-part="glyph" aria-hidden="true">
                    {item.glyph ?? String(index + 1)}
                  </span>
                  ...
                  {open ? (
                    <div data-part="body" role="tabpanel">
                      {item.note ? <span data-part="note">{item.note}</span> : null}
                      <h3 data-part="card-title">{item.title}</h3>
                      <p data-part="text">{item.text}</p>
                      {outcomesLabel ? <p data-part="outcomes-label">{outcomesLabel}</p> : null}
                      <ul data-part="outcomes">
                        {item.outcomes.map((outcome, n) => (
