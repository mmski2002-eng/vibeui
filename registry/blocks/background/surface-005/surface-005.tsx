...
// Жидкий металл: поле складок с нормалью и двумя источниками света —
// френель по краям складок, резкий блик и анизотропные полосы отражения,
// как на полированном хроме. Монохром здесь родной: металл цвета не просит,
// accent — цвет блика. Считает видеокарта в полразрешения.
...
--vibeui-surface-005-c1:light-dark(#bfbfbf,#000000);
--vibeui-surface-005-c2:light-dark(#ececec,#333333);
--vibeui-surface-005-c3:light-dark(#ffffff,#8c8c8c);
--vibeui-surface-005-accent:light-dark(#1a1a1a,#f2f2f2);
--vibeui-surface-005-c4:var(--vibeui-surface-005-accent);
--vibeui-surface-005-ink:light-dark(#000000,#ffffff);
--vibeui-surface-005-ghost:light-dark(color-mix(in oklab,#000000 78%,transparent),color-mix(in oklab,#ffffff 82%,transparent));
--vibeui-surface-005-ghost-soft:light-dark(color-mix(in oklab,#000000 40%,transparent),color-mix(in oklab,#ffffff 45%,transparent));
--vibeui-surface-005-glass:light-dark(color-mix(in oklab,#ffffff 42%,transparent),color-mix(in oklab,#ffffff 8%,transparent));
--vibeui-surface-005-edge:light-dark(color-mix(in oklab,#000000 10%,transparent),color-mix(in oklab,#ffffff 18%,transparent));
...
:where(.dark,[data-theme="dark"]) [data-vibeui-block="surface-005"]{color-scheme:dark}
:where([data-vibeui-block="surface-005"][data-tone="light"]){color-scheme:light}
:where([data-vibeui-block="surface-005"][data-tone="dark"]){color-scheme:dark}
[data-vibeui-block="surface-005"]{
position:relative;display:block;min-width:min(100%,16rem);overflow:hidden;
background:var(--vibeui-surface-005-c1);color:var(--vibeui-surface-005-ink);
...
[data-vibeui-block="surface-005"] [data-part="still"]{
position:absolute;inset:0;pointer-events:none;
background:
radial-gradient(60% 70% at 18% 28%,var(--vibeui-surface-005-c2) 0%,transparent 70%),
radial-gradient(55% 60% at 82% 30%,var(--vibeui-surface-005-c3) 0%,transparent 70%),
radial-gradient(50% 45% at 50% 88%,var(--vibeui-surface-005-c4) 0%,transparent 70%),
var(--vibeui-surface-005-c1);
}
...
[data-vibeui-block="surface-005"] canvas{
position:absolute;inset:0;width:100%;height:100%;display:block;pointer-events:none;
border:0 solid transparent;
border-top-color:var(--vibeui-surface-005-c1);
border-right-color:var(--vibeui-surface-005-c2);
border-bottom-color:var(--vibeui-surface-005-c3);
border-left-color:var(--vibeui-surface-005-c4);
opacity:0;transition:opacity 0.8s ease;
}
[data-vibeui-block="surface-005"][data-drawn="true"] canvas{opacity:1}
...
[data-vibeui-block="surface-005"] [data-part="ghost"]{display:flex;flex-direction:column;flex:1;gap:3rem}
[data-vibeui-block="surface-005"] [data-part="nav"]{display:flex;align-items:center;gap:0.75rem}
[data-vibeui-block="surface-005"] [data-part="mark"]{width:1.5rem;height:1.5rem;flex:none;border-radius:0.375rem;background:var(--vibeui-surface-005-ink)}
[data-vibeui-block="surface-005"] [data-part="nav"] span:not([data-part]){width:3rem;height:0.5rem;border-radius:999px;background:var(--vibeui-surface-005-ghost-soft)}
[data-vibeui-block="surface-005"] [data-part="nav"] span:last-child{margin-inline-start:auto;width:4.5rem;height:1.75rem;border-radius:0.5rem;background:var(--vibeui-surface-005-ink)}
[data-vibeui-block="surface-005"] [data-part="title"]{display:flex;flex-direction:column;gap:0.875rem;max-width:36rem}
[data-vibeui-block="surface-005"] [data-part="title"] span{height:1.25rem;border-radius:999px;background:var(--vibeui-surface-005-ghost);width:90%}
[data-vibeui-block="surface-005"] [data-part="title"] span:nth-child(2){width:62%}
[data-vibeui-block="surface-005"] [data-part="title"] span:nth-child(3){height:0.625rem;width:48%;background:var(--vibeui-surface-005-ghost-soft);margin-top:0.5rem}
[data-vibeui-block="surface-005"] [data-part="cards"]{margin-top:auto;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:1rem}
[data-vibeui-block="surface-005"] [data-part="cards"] span{height:6.5rem;border-radius:1rem;background:var(--vibeui-surface-005-glass);border:1px solid var(--vibeui-surface-005-edge);-webkit-backdrop-filter:blur(18px) saturate(1.2);backdrop-filter:blur(18px) saturate(1.2)}
[data-vibeui-block="surface-005"] [data-part="cards"] span:last-child{display:none}
@container (min-width: 48rem){
[data-vibeui-block="surface-005"] [data-part="frame"]{min-height:34rem;padding-block:2.5rem 3rem}
[data-vibeui-block="surface-005"] [data-part="cards"]{grid-template-columns:repeat(3,minmax(0,1fr))}
[data-vibeui-block="surface-005"] [data-part="cards"] span:last-child{display:block}
}
...
vec2 w=p;
for(int i=0;i<4;i++){float f=float(i);w+=.38*vec2(sin(w.y*1.3+t*.7+f*1.9),cos(w.x*1.1-t*.5+f*1.3));}
return .5*sin(w.x*1.4+t)+.5*cos(w.y*1.2-t*.6)+.35*snoise(w*.8+t*.2);
}
void main(){
...
vec3 n=normalize(vec3(-gx*.45,-gy*.45,1.));
vec3 v=vec3(0.,0.,1.);
vec3 l1=normalize(vec3(-.5,.8,.5));
vec3 l2=normalize(vec3(.7,-.3,.4));
float d1=max(dot(n,l1),0.);
float fres=pow(1.-max(dot(n,v),0.),3.);
float s1=pow(max(dot(reflect(-l1,n),v),0.),mix(10.,70.,p1));
float s2=pow(max(dot(reflect(-l2,n),v),0.),24.);
float env=.5+.5*sin(n.y*9.+n.x*4.);
vec3 base=mix(c1,c2,d1*.8+env*.25);
base=mix(base,c3,fres*.9);
vec3 col=base+c4*(s1*mix(.45,1.,p1)+s2*.22)*mix(.45,1.,p2);
...
const FINISHS = { matte: 0, gloss: 1 } as const
...
/** Жидкий металл: живой фон на WebGL2, один файл, ноль зависимостей. */
export function Surface005({
  ...
  const palette = {
    ...
    ...(colors
      ? {
          "--vibeui-surface-005-c1": colors[0],
          "--vibeui-surface-005-c2": colors[1],
          "--vibeui-surface-005-c3": colors[2],
          "--vibeui-surface-005-c4": colors[3],
        }
      : null),
    ...style,
  } as CSSProperties
