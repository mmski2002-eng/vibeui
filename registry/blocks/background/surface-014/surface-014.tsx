...
// ASCII-поле: картина из символов. Атлас из 14 глифов рисуется один раз в
// canvas2D и уходит в текстуру; шейдер делит холст на ячейки, считает
// яркость живого поля в центре ячейки и берёт глиф по яркости. DOM не
// участвует, символов может быть тысячи — цена одна выборка на пиксель.
...
--vibeui-surface-014-c1:light-dark(#ffffff,#000000);
--vibeui-surface-014-c2:light-dark(#f2f2f2,#1a1a1a);
--vibeui-surface-014-c3:light-dark(#b0b0b0,#5a5a5a);
--vibeui-surface-014-accent:light-dark(#1a1a1a,#f2f2f2);
--vibeui-surface-014-c4:var(--vibeui-surface-014-accent);
--vibeui-surface-014-ink:light-dark(#000000,#ffffff);
--vibeui-surface-014-ghost:light-dark(color-mix(in oklab,#000000 78%,transparent),color-mix(in oklab,#ffffff 82%,transparent));
--vibeui-surface-014-ghost-soft:light-dark(color-mix(in oklab,#000000 40%,transparent),color-mix(in oklab,#ffffff 45%,transparent));
--vibeui-surface-014-glass:light-dark(color-mix(in oklab,#ffffff 42%,transparent),color-mix(in oklab,#ffffff 8%,transparent));
--vibeui-surface-014-edge:light-dark(color-mix(in oklab,#000000 10%,transparent),color-mix(in oklab,#ffffff 18%,transparent));
...
:where(.dark,[data-theme="dark"]) [data-vibeui-block="surface-014"]{color-scheme:dark}
:where([data-vibeui-block="surface-014"][data-tone="light"]){color-scheme:light}
:where([data-vibeui-block="surface-014"][data-tone="dark"]){color-scheme:dark}
[data-vibeui-block="surface-014"]{
position:relative;display:block;min-width:min(100%,16rem);overflow:hidden;
background:var(--vibeui-surface-014-c1);color:var(--vibeui-surface-014-ink);
...
[data-vibeui-block="surface-014"] [data-part="still"]{
position:absolute;inset:0;pointer-events:none;
background:
radial-gradient(60% 70% at 18% 28%,var(--vibeui-surface-014-c2) 0%,transparent 70%),
radial-gradient(55% 60% at 82% 30%,var(--vibeui-surface-014-c3) 0%,transparent 70%),
radial-gradient(50% 45% at 50% 88%,var(--vibeui-surface-014-c4) 0%,transparent 70%),
var(--vibeui-surface-014-c1);
}
...
[data-vibeui-block="surface-014"] canvas{
position:absolute;inset:0;width:100%;height:100%;display:block;pointer-events:none;
border:0 solid transparent;
border-top-color:var(--vibeui-surface-014-c1);
border-right-color:var(--vibeui-surface-014-c2);
border-bottom-color:var(--vibeui-surface-014-c3);
border-left-color:var(--vibeui-surface-014-c4);
opacity:0;transition:opacity 0.8s ease;
}
[data-vibeui-block="surface-014"][data-drawn="true"] canvas{opacity:1}
[data-vibeui-block="surface-014"] [data-part="frame"]{
...
[data-vibeui-block="surface-014"] [data-part="ghost"]{display:flex;flex-direction:column;flex:1;gap:3rem}
[data-vibeui-block="surface-014"] [data-part="nav"]{display:flex;align-items:center;gap:0.75rem}
[data-vibeui-block="surface-014"] [data-part="mark"]{width:1.5rem;height:1.5rem;flex:none;border-radius:0.375rem;background:var(--vibeui-surface-014-ink)}
[data-vibeui-block="surface-014"] [data-part="nav"] span:not([data-part]){width:3rem;height:0.5rem;border-radius:999px;background:var(--vibeui-surface-014-ghost-soft)}
[data-vibeui-block="surface-014"] [data-part="nav"] span:last-child{margin-inline-start:auto;width:4.5rem;height:1.75rem;border-radius:0.5rem;background:var(--vibeui-surface-014-ink)}
[data-vibeui-block="surface-014"] [data-part="title"]{display:flex;flex-direction:column;gap:0.875rem;max-width:36rem}
[data-vibeui-block="surface-014"] [data-part="title"] span{height:1.25rem;border-radius:999px;background:var(--vibeui-surface-014-ghost);width:90%}
[data-vibeui-block="surface-014"] [data-part="title"] span:nth-child(2){width:62%}
[data-vibeui-block="surface-014"] [data-part="title"] span:nth-child(3){height:0.625rem;width:48%;background:var(--vibeui-surface-014-ghost-soft);margin-top:0.5rem}
[data-vibeui-block="surface-014"] [data-part="cards"]{margin-top:auto;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:1rem}
[data-vibeui-block="surface-014"] [data-part="cards"] span{height:6.5rem;border-radius:1rem;background:var(--vibeui-surface-014-glass);border:1px solid var(--vibeui-surface-014-edge);-webkit-backdrop-filter:blur(18px) saturate(1.2);backdrop-filter:blur(18px) saturate(1.2)}
[data-vibeui-block="surface-014"] [data-part="cards"] span:last-child{display:none}
@container (min-width: 48rem){
[data-vibeui-block="surface-014"] [data-part="frame"]{min-height:34rem;padding-block:2.5rem 3rem}
[data-vibeui-block="surface-014"] [data-part="cards"]{grid-template-columns:repeat(3,minmax(0,1fr))}
[data-vibeui-block="surface-014"] [data-part="cards"] span:last-child{display:block}
}
...
float t=time*.35;
vec2 cellPx=vec2(p1*.6,p1);
vec2 g=gl_FragCoord.xy/cellPx;
vec2 id=floor(g);vec2 f=fract(g);
vec2 cp=(id+.5)*cellPx/resolution;cp.x*=r;
float v=.5+.5*snoise(cp*1.8+vec2(t*.4,t*.2));
v=v*.65+.35*(.5+.5*snoise(cp*4.-t*.3));
vec2 lc=vec2(r*.6,.5)+.2*vec2(sin(t*.7),cos(t*.5));
v*=smoothstep(1.4,.1,length(cp-lc))*mix(.7,1.,p2);
float idx=floor(clamp(v,0.,.999)*14.);
float a=texture(tex,vec2((idx+f.x)/14.,1.-f.y)).r;
vec3 col=mix(c1,mix(c3,c4,v),a*mix(.7,1.,p2));
color=vec4(col,1.);
}`

const SPEEDS = { still: 0, calm: 1, lively: 2.4 } as const
const INTENSITIES = { faint: 0.3, soft: 0.55, bright: 1 } as const
const CELLS = { small: 12, large: 18 } as const
...
/** ASCII-поле: живой фон на WebGL2, один файл, ноль зависимостей. */
export function Surface014({
  children,
  speed = "calm",
  cell = "small",
  intensity = "soft",
  ...
  useEffect(() => {
    ...
    if (ink) {
      ...
      ink.fillStyle = "#fff"
      ink.font = "16px ui-monospace, SFMono-Regular, Menlo, Consolas, monospace"
      ink.textAlign = "center"
      ink.textBaseline = "middle"

      for (let index = 0; index < glyphs.length; index += 1) {
        ink.fillText(glyphs[index], index * 12 + 6, 11)
      }
    }
  ...
  const palette = {
    ...(accent ? { "--vibeui-surface-014-accent": accent } : null),
    ...(colors
      ? {
          "--vibeui-surface-014-c1": colors[0],
          "--vibeui-surface-014-c2": colors[1],
          "--vibeui-surface-014-c3": colors[2],
          "--vibeui-surface-014-c4": colors[3],
        }
      : null),
    ...style,
  } as CSSProperties
