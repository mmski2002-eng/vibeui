...
// Аврора: несколько светящихся лент с ядром и мягким ореолом дрейфуют по
// синусоиде с шумом — как полярное сияние, но в одной палитре, без радуги.
// Ленты смешиваются с фоном лерпом, а не сложением, поэтому на светлой теме
// они темнеют, а не исчезают. Считает видеокарта в 3/4 разрешения.
...
--vibeui-surface-007-c1:light-dark(#f2f2f2,#000000);
--vibeui-surface-007-c2:light-dark(#ffffff,#1a1a1a);
--vibeui-surface-007-c3:light-dark(#8c8c8c,#6e6e6e);
--vibeui-surface-007-accent:light-dark(#1a1a1a,#f2f2f2);
--vibeui-surface-007-c4:var(--vibeui-surface-007-accent);
--vibeui-surface-007-ink:light-dark(#000000,#ffffff);
--vibeui-surface-007-ghost:light-dark(color-mix(in oklab,#000000 78%,transparent),color-mix(in oklab,#ffffff 82%,transparent));
--vibeui-surface-007-ghost-soft:light-dark(color-mix(in oklab,#000000 40%,transparent),color-mix(in oklab,#ffffff 45%,transparent));
--vibeui-surface-007-glass:light-dark(color-mix(in oklab,#ffffff 42%,transparent),color-mix(in oklab,#ffffff 8%,transparent));
--vibeui-surface-007-edge:light-dark(color-mix(in oklab,#000000 10%,transparent),color-mix(in oklab,#ffffff 18%,transparent));
...
:where(.dark,[data-theme="dark"]) [data-vibeui-block="surface-007"]{color-scheme:dark}
:where([data-vibeui-block="surface-007"][data-tone="light"]){color-scheme:light}
:where([data-vibeui-block="surface-007"][data-tone="dark"]){color-scheme:dark}
[data-vibeui-block="surface-007"]{
position:relative;display:block;min-width:min(100%,16rem);overflow:hidden;
background:var(--vibeui-surface-007-c1);color:var(--vibeui-surface-007-ink);
...
[data-vibeui-block="surface-007"] [data-part="still"]{
position:absolute;inset:0;pointer-events:none;
background:
radial-gradient(60% 70% at 18% 28%,var(--vibeui-surface-007-c2) 0%,transparent 70%),
radial-gradient(55% 60% at 82% 30%,var(--vibeui-surface-007-c3) 0%,transparent 70%),
radial-gradient(50% 45% at 50% 88%,var(--vibeui-surface-007-c4) 0%,transparent 70%),
var(--vibeui-surface-007-c1);
}
...
[data-vibeui-block="surface-007"] canvas{
position:absolute;inset:0;width:100%;height:100%;display:block;pointer-events:none;
border:0 solid transparent;
border-top-color:var(--vibeui-surface-007-c1);
border-right-color:var(--vibeui-surface-007-c2);
border-bottom-color:var(--vibeui-surface-007-c3);
border-left-color:var(--vibeui-surface-007-c4);
opacity:0;transition:opacity 0.8s ease;
}
[data-vibeui-block="surface-007"][data-drawn="true"] canvas{opacity:1}
...
[data-vibeui-block="surface-007"] [data-part="ghost"]{display:flex;flex-direction:column;flex:1;gap:3rem}
[data-vibeui-block="surface-007"] [data-part="nav"]{display:flex;align-items:center;gap:0.75rem}
[data-vibeui-block="surface-007"] [data-part="mark"]{width:1.5rem;height:1.5rem;flex:none;border-radius:0.375rem;background:var(--vibeui-surface-007-ink)}
[data-vibeui-block="surface-007"] [data-part="nav"] span:not([data-part]){width:3rem;height:0.5rem;border-radius:999px;background:var(--vibeui-surface-007-ghost-soft)}
[data-vibeui-block="surface-007"] [data-part="nav"] span:last-child{margin-inline-start:auto;width:4.5rem;height:1.75rem;border-radius:0.5rem;background:var(--vibeui-surface-007-ink)}
[data-vibeui-block="surface-007"] [data-part="title"]{display:flex;flex-direction:column;gap:0.875rem;max-width:36rem}
[data-vibeui-block="surface-007"] [data-part="title"] span{height:1.25rem;border-radius:999px;background:var(--vibeui-surface-007-ghost);width:90%}
[data-vibeui-block="surface-007"] [data-part="title"] span:nth-child(2){width:62%}
[data-vibeui-block="surface-007"] [data-part="title"] span:nth-child(3){height:0.625rem;width:48%;background:var(--vibeui-surface-007-ghost-soft);margin-top:0.5rem}
[data-vibeui-block="surface-007"] [data-part="cards"]{margin-top:auto;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:1rem}
[data-vibeui-block="surface-007"] [data-part="cards"] span{height:6.5rem;border-radius:1rem;background:var(--vibeui-surface-007-glass);border:1px solid var(--vibeui-surface-007-edge);-webkit-backdrop-filter:blur(18px) saturate(1.2);backdrop-filter:blur(18px) saturate(1.2)}
[data-vibeui-block="surface-007"] [data-part="cards"] span:last-child{display:none}
@container (min-width: 48rem){
[data-vibeui-block="surface-007"] [data-part="frame"]{min-height:34rem;padding-block:2.5rem 3rem}
[data-vibeui-block="surface-007"] [data-part="cards"]{grid-template-columns:repeat(3,minmax(0,1fr))}
[data-vibeui-block="surface-007"] [data-part="cards"] span:last-child{display:block}
}
...
float t=time*.25;
vec3 col=mix(c1,c2,smoothstep(.2,0.,p.y)*.6);
float n=p1;
for(int i=0;i<6;i++){
float f=float(i);if(f>=n)break;
float k=f/max(n-1.,1.);
float y=.28+.5*k+.14*sin(p.x*1.4+t*.6+f*1.3)+.1*snoise(vec2(p.x*1.2-t*.2,f*3.7));
float d=abs(p.y-y);
float w=.01+.022*(.5+.5*sin(p.x*2.+f+t));
float g=exp(-d*d/(w*w));
float glow=exp(-d*7.)*.4;
vec3 rc=mix(c3,c4,k);
col=mix(col,rc,clamp(g*.85+glow,0.,1.)*p2);
}
...
const BANDSS = { few: 3, many: 6 } as const
...
/** Аврора: живой фон на WebGL2, один файл, ноль зависимостей. */
export function Surface007({
  ...
  const palette = {
    ...
    ...(colors
      ? {
          "--vibeui-surface-007-c1": colors[0],
          "--vibeui-surface-007-c2": colors[1],
          "--vibeui-surface-007-c3": colors[2],
          "--vibeui-surface-007-c4": colors[3],
        }
      : null),
    ...style,
  } as CSSProperties
