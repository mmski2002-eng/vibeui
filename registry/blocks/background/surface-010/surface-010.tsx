...
// Нейро-шум: два слоя simplex-шума, искажённые сами собой, дают светящиеся
// прожилки там, где шум проходит через ноль — как нейросеть, мрамор или
// молнии под кожей. Прожилки — accent, второй слой тусклее. Считает
// видеокарта в 3/4 разрешения: линиям нужна резкость, но не каждый пиксель.
...
--vibeui-surface-010-c1:light-dark(#ffffff,#000000);
--vibeui-surface-010-c2:light-dark(#f2f2f2,#1a1a1a);
--vibeui-surface-010-c3:light-dark(#d9d9d9,#3a3a3a);
--vibeui-surface-010-accent:light-dark(#1a1a1a,#f2f2f2);
--vibeui-surface-010-c4:var(--vibeui-surface-010-accent);
--vibeui-surface-010-ink:light-dark(#000000,#ffffff);
--vibeui-surface-010-ghost:light-dark(color-mix(in oklab,#000000 78%,transparent),color-mix(in oklab,#ffffff 82%,transparent));
--vibeui-surface-010-ghost-soft:light-dark(color-mix(in oklab,#000000 40%,transparent),color-mix(in oklab,#ffffff 45%,transparent));
--vibeui-surface-010-glass:light-dark(color-mix(in oklab,#ffffff 42%,transparent),color-mix(in oklab,#ffffff 8%,transparent));
--vibeui-surface-010-edge:light-dark(color-mix(in oklab,#000000 10%,transparent),color-mix(in oklab,#ffffff 18%,transparent));
...
:where(.dark,[data-theme="dark"]) [data-vibeui-block="surface-010"]{color-scheme:dark}
:where([data-vibeui-block="surface-010"][data-tone="light"]){color-scheme:light}
:where([data-vibeui-block="surface-010"][data-tone="dark"]){color-scheme:dark}
[data-vibeui-block="surface-010"]{
position:relative;display:block;min-width:min(100%,16rem);overflow:hidden;
background:var(--vibeui-surface-010-c1);color:var(--vibeui-surface-010-ink);
...
[data-vibeui-block="surface-010"] [data-part="still"]{
position:absolute;inset:0;pointer-events:none;
background:
radial-gradient(60% 70% at 18% 28%,var(--vibeui-surface-010-c2) 0%,transparent 70%),
radial-gradient(55% 60% at 82% 30%,var(--vibeui-surface-010-c3) 0%,transparent 70%),
radial-gradient(50% 45% at 50% 88%,var(--vibeui-surface-010-c4) 0%,transparent 70%),
var(--vibeui-surface-010-c1);
}
...
[data-vibeui-block="surface-010"] canvas{
position:absolute;inset:0;width:100%;height:100%;display:block;pointer-events:none;
border:0 solid transparent;
border-top-color:var(--vibeui-surface-010-c1);
border-right-color:var(--vibeui-surface-010-c2);
border-bottom-color:var(--vibeui-surface-010-c3);
border-left-color:var(--vibeui-surface-010-c4);
opacity:0;transition:opacity 0.8s ease;
}
[data-vibeui-block="surface-010"][data-drawn="true"] canvas{opacity:1}
...
[data-vibeui-block="surface-010"] [data-part="ghost"]{display:flex;flex-direction:column;flex:1;gap:3rem}
[data-vibeui-block="surface-010"] [data-part="nav"]{display:flex;align-items:center;gap:0.75rem}
[data-vibeui-block="surface-010"] [data-part="mark"]{width:1.5rem;height:1.5rem;flex:none;border-radius:0.375rem;background:var(--vibeui-surface-010-ink)}
[data-vibeui-block="surface-010"] [data-part="nav"] span:not([data-part]){width:3rem;height:0.5rem;border-radius:999px;background:var(--vibeui-surface-010-ghost-soft)}
[data-vibeui-block="surface-010"] [data-part="nav"] span:last-child{margin-inline-start:auto;width:4.5rem;height:1.75rem;border-radius:0.5rem;background:var(--vibeui-surface-010-ink)}
[data-vibeui-block="surface-010"] [data-part="title"]{display:flex;flex-direction:column;gap:0.875rem;max-width:36rem}
[data-vibeui-block="surface-010"] [data-part="title"] span{height:1.25rem;border-radius:999px;background:var(--vibeui-surface-010-ghost);width:90%}
[data-vibeui-block="surface-010"] [data-part="title"] span:nth-child(2){width:62%}
[data-vibeui-block="surface-010"] [data-part="title"] span:nth-child(3){height:0.625rem;width:48%;background:var(--vibeui-surface-010-ghost-soft);margin-top:0.5rem}
[data-vibeui-block="surface-010"] [data-part="cards"]{margin-top:auto;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:1rem}
[data-vibeui-block="surface-010"] [data-part="cards"] span{height:6.5rem;border-radius:1rem;background:var(--vibeui-surface-010-glass);border:1px solid var(--vibeui-surface-010-edge);-webkit-backdrop-filter:blur(18px) saturate(1.2);backdrop-filter:blur(18px) saturate(1.2)}
[data-vibeui-block="surface-010"] [data-part="cards"] span:last-child{display:none}
@container (min-width: 48rem){
[data-vibeui-block="surface-010"] [data-part="frame"]{min-height:34rem;padding-block:2.5rem 3rem}
[data-vibeui-block="surface-010"] [data-part="cards"]{grid-template-columns:repeat(3,minmax(0,1fr))}
[data-vibeui-block="surface-010"] [data-part="cards"] span:last-child{display:block}
}
...
float t=time*.25;
vec2 q=p;
for(int i=0;i<3;i++){float f=float(i);q+=.3*vec2(snoise(q*1.2+t*.25+f),snoise(q*1.2-t*.2+f*2.));}
float n1=snoise(q*1.5+t*.15);
float n2=snoise(q*2.6-t*.1+4.);
float v=pow(1.-abs(n1),mix(14.,6.,p1));
float v2=pow(1.-abs(n2),10.)*mix(.4,.8,p1);
float vig=smoothstep(1.9,.4,length(p-vec2(r,1.)));
vec3 col=mix(c1,c2,.5*vig);
col=mix(col,c3,v2*vig*p2);
col=mix(col,c4,v*vig*mix(.55,1.,p2));
...
const DENSITYS = { sparse: 0, dense: 1 } as const
...
/** Нейро-шум: живой фон на WebGL2, один файл, ноль зависимостей. */
export function Surface010({
  ...
  const palette = {
    ...
    ...(colors
      ? {
          "--vibeui-surface-010-c1": colors[0],
          "--vibeui-surface-010-c2": colors[1],
          "--vibeui-surface-010-c3": colors[2],
          "--vibeui-surface-010-c4": colors[3],
        }
      : null),
    ...style,
  } as CSSProperties
