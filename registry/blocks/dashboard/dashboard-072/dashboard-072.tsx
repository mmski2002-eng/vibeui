...
--vibeui-dashboard-072-bg:oklch(0.985 0.003 285);
--vibeui-dashboard-072-card:oklch(1 0 0);
--vibeui-dashboard-072-fg:oklch(0.21 0.014 285);
--vibeui-dashboard-072-muted:oklch(0.55 0.014 285);
--vibeui-dashboard-072-border:oklch(0.91 0.006 285);
--vibeui-dashboard-072-accent:oklch(0.52 0.16 285);
--vibeui-dashboard-072-soft:oklch(0.965 0.02 285);
--vibeui-dashboard-072-control:oklch(0.6 0.02 285);
--vibeui-dashboard-072-win:oklch(0.58 0.13 155);
...
border:1px solid color-mix(in oklab,var(--vibeui-dashboard-072-accent) 22%,white);
}
...
background:color-mix(in oklab,var(--vibeui-dashboard-072-control) 45%,white);
}
[data-vibeui-block="dashboard-072"] [data-win="true"] [data-part="span"]{
background:color-mix(in oklab,var(--vibeui-dashboard-072-win) 40%,white);
}
...
background:var(--vibeui-dashboard-072-accent);color:oklch(1 0 0);border-color:transparent;
}
        ...
        <div data-part="shell">
          ...
          <div data-part="verdict">
            ...
            <span
              ...
              aria-label="Прогресс набора данных"
            >
              <span
                style={{
            ...
            <span>
              идёт {experiment.days} из {experiment.needDays} дней
            </span>
          </div>
          ...
          <div data-part="chart">
            {experiment.variants.map((variant) => (
              ...
              >
                <p data-part="who">
                  <b>{variant.name}</b>
                  {variant.control ? (
                    <span data-part="tag">контроль</span>
                  ) : null}
                  <span>{variant.traffic} % трафика</span>
                </p>

                <div
                  data-part="scale"
                  role="img"
                  aria-label={`${variant.name}: конверсия ${variant.rate} процента, интервал от ${variant.low} до ${variant.high}`}
                >
                  <span
                    data-part="span"
                    style={{
                ...
                <p data-part="nums">
                  <span>
                    <b>{variant.rate.toFixed(1)} %</b> конверсия
                  </span>
                  <span>
                    {variant.low.toFixed(2)}–{variant.high.toFixed(2)} %
                  </span>
                  <span>
                    {variant.conversions} из{" "}
                    {variant.visitors.toLocaleString("ru-RU")}
                  </span>
                </p>
              </div>
            ))}
            ...
            <div data-part="axis">
              <span>{from.toFixed(1)} %</span>
              <span>{((from + to) / 2).toFixed(1)} %</span>
              <span>{to.toFixed(1)} %</span>
            </div>
          </div>
          ...
          <p data-part="note">
            Отрезок — доверительный интервал конверсии, точка — сама оценка.
            Пока отрезки веток перекрываются, победителя нет, каким бы
            заманчивым ни выглядело различие средних.
          </p>
        </div>
      </section>
    </>
