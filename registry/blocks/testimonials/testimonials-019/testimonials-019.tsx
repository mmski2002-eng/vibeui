...
import { Card109 } from "@/registry/components/card/card-109/card-109"
...
[data-vibeui-block="testimonials-019"] [data-part="card"]{width:min(22rem,86cqi);margin-left:calc(min(22rem,86cqi) / -2)}
...
[data-vibeui-block="testimonials-019"] [data-part="arrows"] button:focus-visible,[data-vibeui-block="testimonials-019"] [data-part="dot"]:focus-visible{outline:2px solid var(--vibeui-testimonials-019-cyan);outline-offset:3px}
        ...
        <div data-part="shell">
          ...
          <div data-part="stage" onKeyDown={(event) => (event.key === "ArrowRight" ? go(active + 1) : event.key === "ArrowLeft" ? go(active - 1) : null)}>
            {items.map((item, index) => {
              ...
              return (
                <Card109 key={item.name + index} data-part="card" name={item.name} image={item.image} imageAlt={item.imageAlt} after={item.after} quote={item.quote} meta={item.meta} rating={item.rating} ratingLine={ratingLine} data-active={index === active} data-hidden={abs > 2 ? "true" : undefined} aria-hidden={index === active ? undefined : "true"} tabIndex={index === active ? 0 : -1} style={{ ["--vibeui-testimonials-019-d" as string]: d, ["--vibeui-testimonials-019-abs" as string]: abs }} onClick={() => go(index)} accent={accent} />
              )
            })}
          </div>
          <ul data-part="dots">
