...
[data-vibeui-block="hero-035"] [data-part="face"]{position:relative;width:100%;aspect-ratio:1;--vibeui-hero-035-ex:0;--vibeui-hero-035-ey:0;border-radius:58% 42% 55% 45% / 48% 56% 44% 52%;background:radial-gradient(closest-side,color-mix(in oklab,var(--vibeui-hero-035-accent) 28%,transparent),transparent);animation:vibeui-hero-035-morph 12s ease-in-out infinite alternate}
...
[data-vibeui-block="hero-035"] [data-part="face"] img{position:absolute;inset:0;width:100%;height:100%;object-fit:contain;object-position:50% 100%;opacity:0;transform:translate(calc(var(--vibeui-hero-035-ex) * -1.5%),calc(var(--vibeui-hero-035-ey) * -1.5%)) scale(1.06);transition:opacity .6s cubic-bezier(.2,.8,.2,1),transform .5s ease-out}
...
const DEFAULT_PETS: Hero035Pet[] = [
  { key: "cat", label: "Кот", eyebrow: "Ветклиника и груминг на Соколе · 24/7", title: "Лечим так, что *кот не заметит*", lede: "Тихие кабинеты без собачьего лая, приём по записи без очереди и врачи, которые сначала гладят, потом смотрят. Прививки, зубы, стерилизация — всё в одном месте.", bubble: "Мя. Меня даже не держали.", image: "/demo/vet/cat-face.png", imageAlt: "Рыжий кот смотрит в камеру" },
  { key: "dog", label: "Собака", eyebrow: "Ветклиника и груминг на Соколе · 24/7", title: "Лечим так, что *хвост не перестаёт*", lede: "Отдельный вход для собак, весы прямо в холле и лакомство после укола. Ортопед, стоматолог, груминг — и никто не будет гладить против шерсти.", bubble: "Гав. Тут дают вкусняшки.", image: "/demo/vet/dog-face.png", imageAlt: "Корги с высунутым языком" },
  { key: "rabbit", label: "Кролик", eyebrow: "Ветклиника и груминг на Соколе · 24/7", title: "Лечим так, что *уши не вянут*", lede: "Врач по грызунам и кроликам каждый день, а не «по четвергам». Зубы, ЖКТ, стрижка когтей — быстро, тихо и без стресса для длинноухих.", bubble: "Морковку взяла с собой.", image: "/demo/vet/rabbit-face.png", imageAlt: "Серый вислоухий кролик" },
]
