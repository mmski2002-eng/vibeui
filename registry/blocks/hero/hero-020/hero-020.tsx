...
export type Hero020Props = {
  /** Фото на весь экран. Служит постером, пока грузится видео, и запасом без него. */
  image?: string
  imageAlt?: string
  /** Фоновое видео (mp4). Играет вместо наезда по фото: autoplay, muted, loop. */
  video?: string
  /** Тот же ролик в webm — уходит первым источником, mp4 остаётся запасным. */
  videoWebm?: string
  eyebrow?: string
  title?: string
  lede?: string
...
[data-vibeui-block="hero-020"] [data-part="submit"]{align-self:end}
...
/* Видео двигается само — наезд по нему выключаем, чтобы движения не удваивались. */
[data-vibeui-block="hero-020"] video[data-part="picture"]{animation:none}
...
[data-vibeui-block="hero-020"] [data-part="shell"]{min-height:min(88vh,52rem);padding:8rem 2rem 3rem}
  ...
  return (
    <>
      ...
      <section data-vibeui-block="hero-020" data-tone={tone === "auto" ? undefined : tone} className={className} style={palette}>
        {video ? (
          <video data-part="picture" autoPlay muted loop playsInline preload="metadata" poster={image || undefined} aria-hidden="true">
            {videoWebm ? <source src={videoWebm} type="video/webm" /> : null}
            <source src={video} type="video/mp4" />
          </video>
        ) : image ? (
          <img data-part="picture" src={image} alt={imageAlt} />
        ) : (
          <span data-part="picture" data-empty="" aria-hidden="true" />
        )}
        ...
        <div data-part="shell">
          ...
          <form data-part="search" method="get" action={action}>
            ...
            <div data-part="fields">
              <Select001 name="type" size="lg" label={typeLabel} placeholder="" options={types.map((item) => ({ value: item, label: item }))} accent={accent} />
              <Select001 name="district" size="lg" label={districtLabel} placeholder="" options={districts.map((item) => ({ value: item, label: item }))} accent={accent} />
              <Select001 name="budget" size="lg" label={budgetLabel} placeholder="" options={budgets.map((item) => ({ value: item, label: item }))} accent={accent} />
