...
export async function AccountConnect({ locale }: { locale: Locale }) {
  ...
  const day = (value: Date) =>
    value.toLocaleDateString(locale === "en" ? "en-GB" : "ru-RU")
  ...
  return (
    <>
        ...
        <Step index={1} title={t.step1} note={t.step1Free}>
          {sample ? (
            <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:items-center">
              <code className="bg-shell-elevated border-shell-border text-shell-fg min-w-0 flex-1 overflow-x-auto rounded-lg border px-3 py-2 font-mono text-xs whitespace-nowrap">
                {sample}
              </code>
              <CopyButton
                value={sample}
      ...
      <section className="mt-10">
        <h2 className="text-shell-fg font-medium">{t.keyTitle}</h2>
        <p className="text-shell-muted mt-1.5 mb-4 max-w-2xl text-sm leading-relaxed">
          {t.keyLead}
        </p>
        ...
        <TokenPanel
          ...
          tokens={tokens.map((token) => ({
            id: token.id,
            prefix: token.prefix,
            createdAt: day(token.createdAt),
            lastUsedAt: token.lastUsedAt ? day(token.lastUsedAt) : null,
...
function Step({
  ...
  return (
    <li className="border-shell-border bg-shell-panel rounded-2xl border p-5">
      <div className="flex items-start gap-3">
        <span className="border-shell-border text-shell-fg mt-0.5 flex size-6 shrink-0 items-center justify-center rounded-full border text-xs tabular-nums">
          {index}
        </span>
