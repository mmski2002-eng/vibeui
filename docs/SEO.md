...
Отдельно закрыты сценарии с query (`/scenarios/*?`). Выбор блоков живёт в
параметрах адреса, и краулер накапливал их, пока не разошёлся на 114 550
адресов за сутки — каждый полный серверный рендер на единственном ядре
(`docs/HOSTING-AUDIT.md`). Барьеров четыре, и нужны все:

| Слой | Где | Что делает |
| --- | --- | --- |
| `Disallow` | `app/robots.ts` | послушный краулер не скачивает |
| `Clean-param` | `app/robots.ts` | Яндекс снимает адреса из очереди |
| `rel="nofollow"` | `components/pages/scenario-page.tsx` | закрывает источник ссылок |
| `noindex, nofollow` | `app/scenarios/[slug]/page.tsx` | для тех, кто robots.txt игнорирует |

Сам сценарий без параметров остаётся открытым и индексируемым.

Ни один из четырёх не снимает нагрузку с сервера: `noindex` печатается после
того, как рендер уже случился, а `robots.txt` — договорённость, не барьер.
Защита от бота, который её игнорирует, живёт на уровне nginx и не сделана.
