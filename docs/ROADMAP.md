...
- [x] проверено на проде: регистрация, вход, кабинет, лимит вошедшего
      (100 уникальных, повторы бесплатны) и анонима (10), отказ с текстом
      и ссылкой на тариф;
- [x] Postfix с DKIM на сервере, приложение отдаёт письма ему;
- [ ] PTR и записи SPF/DKIM/DMARC в DNS — без них письма не доходят
      (docs/MAIL-SETUP.md);
