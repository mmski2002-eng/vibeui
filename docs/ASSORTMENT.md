...
шестнадцать (`buttons`, `accordion`, `alert`, `dialog`, `aspect`,
`autocomplete`, `avatar`, `badge`, `breadcrumb`, `calendar`, `inputs`,
`display`, `feedback`, `navigation`, `tables`, `charts`). Таксономия закрыта:
новых категорий компонентов не требуется.
...
11. `accordion` — категория закрыта полностью, 11 вариантов: нативный,
...
12. `display` — первая волна закрыта: `badge-001` (цвет только у точки),
...
13. `feedback` — первая волна закрыта: `alert-001` (тон в полосе, не в
...
14. `navigation` — первая волна закрыта: `tabs-001` (полная клавиатура),
