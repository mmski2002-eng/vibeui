...
тринадцать (`buttons`, `accordion`, `alert`, `dialog`, `aspect`,
`autocomplete`, `avatar`, `inputs`, `display`, `feedback`, `navigation`,
`tables`, `charts`); для полного ассортимента нужна ещё одна — `calendar`.
...
8. `accordion` — категория закрыта полностью, 11 вариантов: нативный,
...
9. `display` — первая волна закрыта: `badge-001` (цвет только у точки),
...
10. `feedback` — первая волна закрыта: `alert-001` (тон в полосе, не в
...
11. `navigation` — первая волна закрыта: `tabs-001` (полная клавиатура),
