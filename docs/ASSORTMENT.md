...
девятнадцать (`buttons`, `accordion`, `alert`, `dialog`, `aspect`,
`autocomplete`, `avatar`, `badge`, `breadcrumb`, `calendar`, `card`,
`carousel`, `checkbox`, `inputs`, `display`, `feedback`, `navigation`,
`tables`, `charts`).
...
14. `accordion` — категория закрыта полностью, 11 вариантов: нативный,
...
15. `display` — первая волна закрыта: `badge-001` (цвет только у точки),
...
16. `feedback` — первая волна закрыта: `alert-001` (тон в полосе, не в
...
17. `navigation` — первая волна закрыта: `tabs-001` (полная клавиатура),
...
20. `charts` — 10 items в двух волнах. Первая: линия на SVG, столбцы на
    ...
    двух периодов.
21. Дальше: блоки `dashboard` и `ai`, вторые волны в inputs, display,
