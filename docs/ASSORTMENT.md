...
семнадцать (`buttons`, `accordion`, `alert`, `dialog`, `aspect`,
`autocomplete`, `avatar`, `badge`, `breadcrumb`, `calendar`, `card`,
`inputs`, `display`, `feedback`, `navigation`, `tables`, `charts`).
...
12. `accordion` — категория закрыта полностью, 11 вариантов: нативный,
...
13. `display` — первая волна закрыта: `badge-001` (цвет только у точки),
...
14. `feedback` — первая волна закрыта: `alert-001` (тон в полосе, не в
...
15. `navigation` — первая волна закрыта: `tabs-001` (полная клавиатура),
