...
восемнадцать (`buttons`, `accordion`, `alert`, `dialog`, `aspect`,
`autocomplete`, `avatar`, `badge`, `breadcrumb`, `calendar`, `card`,
`carousel`, `inputs`, `display`, `feedback`, `navigation`, `tables`,
`charts`).
...
13. `accordion` — категория закрыта полностью, 11 вариантов: нативный,
...
14. `display` — первая волна закрыта: `badge-001` (цвет только у точки),
...
15. `feedback` — первая волна закрыта: `alert-001` (тон в полосе, не в
...
16. `navigation` — первая волна закрыта: `tabs-001` (полная клавиатура),
...
19. `charts` — 10 items в двух волнах. Первая: линия на SVG, столбцы на
    ...
    двух периодов.
20. Дальше: блоки `dashboard` и `ai`, вторые волны в inputs, display,
