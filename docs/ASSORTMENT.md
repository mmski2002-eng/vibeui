...
двенадцать (`buttons`, `accordion`, `alert`, `dialog`, `aspect`,
`autocomplete`, `inputs`, `display`, `feedback`, `navigation`, `tables`,
`charts`); для полного ассортимента нужна ещё одна — `calendar`.
...
7. `accordion` — категория закрыта полностью, 11 вариантов: нативный,
...
8. `display` — первая волна закрыта: `badge-001` (цвет только у точки),
...
9. `feedback` — первая волна закрыта: `alert-001` (тон в полосе, не в
...
10. `navigation` — первая волна закрыта: `tabs-001` (полная клавиатура),
