...
пятнадцать (`buttons`, `accordion`, `alert`, `dialog`, `aspect`,
`autocomplete`, `avatar`, `badge`, `breadcrumb`, `inputs`, `display`,
`feedback`, `navigation`, `tables`, `charts`); для полного ассортимента нужна
ещё одна — `calendar`.
...
10. `accordion` — категория закрыта полностью, 11 вариантов: нативный,
...
11. `display` — первая волна закрыта: `badge-001` (цвет только у точки),
...
12. `feedback` — первая волна закрыта: `alert-001` (тон в полосе, не в
...
13. `navigation` — первая волна закрыта: `tabs-001` (полная клавиатура),
