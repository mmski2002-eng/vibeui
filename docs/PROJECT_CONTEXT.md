...
- [SEARCH.md](SEARCH.md) — поиск по каталогу: устройство, словарь синонимов,
  что делает новый item находимым.
...
- [../lib/search/dictionary.ts](../lib/search/dictionary.ts) — синонимы и стемминг поиска.
- [../lib/search/engine.ts](../lib/search/engine.ts) — индекс MiniSearch и ранжирование.
...
- **Наполнение каталога волнами.** 651 item в 39 реестрах: компоненты —
  `buttons` (32), `accordion` (11), `alert` (20), `dialog` (14), `alertdialog` (14), `aspect` (8),
  `autocomplete` (12), `avatar` (20), `badge` (25), `breadcrumb` (15), `buttongroup` (12),
  `calendar` (20), `card` (21), `carousel` (8), `checkbox` (22), `menu` (19),
  `codeblock` (14), `combobox` (12), `datagrid` (12), `eventcalendar` (5),
  `inputs` (85), `display` (77),
  `feedback` (31), `navigation` (58), `tables` (10), `charts` (31); блоки — `hero` (3), `features`, `pricing`, `navbar`, `cta`,
  `faq`, `testimonials`, `footer`, `dashboard` (14), `ai` (3), `commerce` (6), `auth` (6), `solutions` (6). Вместе с hero, features и pricing блоки
...
У сетки каталога два режима, переключатель стоит в строке инструментов
справа от поиска (`components/catalog/catalog-chrome.tsx`):

- **Крупное превью** — как было: одна-две карточки в ряд, живой компонент
  в полном кадре, панель настройки и кнопки выдачи на месте;
- **Обзор вариантов** — три-четыре колонки, кадр ужат, панель настройки и
  кнопки скрыты, зато под названием видны три тега-признака.

Зачем: в кнопках 76 вариантов, в первых экранах 19, и один первый экран
занимает почти всю высоту окна — сравнить их подряд было нельзя. В обзоре
в поле зрения девять вариантов вместо одного.

Режим живёт в `sessionStorage` под ключом `vibeui-grid-view:<kind>` и
переживает переход между категориями. Он не путается с «Компактным видом»
над списком слева: тот уплотняет боковое меню, а этот — саму витрину.

Реализация — CSS по атрибуту `data-grid-view` на обёртке контента, живой
рендер тот же самый: мельче только кадр, а не источник.

Две грабли, на которые тут наступаешь:

1. **Слои Tailwind.** Показ и размеры частей карточки (`data-part="toolbar"
| "actions" | "facts" | "frame"`) заданы в `app/globals.css`, а не
   утилитами: слой `utilities` идёт после `components` и перебивает правила
   режима независимо от специфичности. По этой причине из разметки убраны
   `flex`, `hidden`, `min-h-44` и `p-6 lg:px-8 lg:py-10` — вернёте их, и
   обзор перестанет ужимать карточки.
2. **Специфичность внутри самого блока.** Все правила колонок держат
   одинаковую форму селектора: либо `:not([data-single])`, либо
   `[data-single]`. Если добавить `:not(...)` только части из них,
   правило для узкого экрана окажется специфичнее правила для широкого и
   перебьёт его на любой ширине — витрина застрянет на двух колонках.

Высота карточек в обзоре считается по содержимому: `align-items: start`
вместо `stretch`, иначе ряд принимает высоту самой громоздкой карточки, и
мелкая кнопка занимает столько же места, сколько таблица.

Высота кадра секции в обзоре ограничена `max-height`, потому что её
задаёт JS: блок рендерится в свои 1280px и вписывается масштабом, иначе
первый экран остался бы семисотпиксельным даже в узкой колонке.
...
Поиск глобальный и серверный. Ищет `searchCatalog()` в `lib/search/engine.ts`
по индексу MiniSearch, построенному из всего registry — по одному индексу на
язык, один раз на процесс. Подсказки под полем ввода отдаёт
`app/api/search/route.ts`, полную выдачу — `app/search/page.tsx`
(единственная динамическая страница каталога). Клиентская часть — только
поле `components/catalog/search-box.tsx`; индекса в браузере нет.

Открытый раздел (компоненты / блоки / анимации) поиск **не сужает**, а лишь
поднимает своё в выдаче. До переделки поиск фильтровал карточки открытой
страницы по `data-search`, и запрос «тарифы» на `/components` отвечал
«ничего не найдено», хотя категория `pricing` есть в блоках. Возвращать ту
схему нельзя.

Русские запросы приводятся к основам грубым стеммером, оценочные слова
(«красивая») отсеиваются, синонимы («цены» → `pricing`, «форма заявки» →
`contact`) лежат явной таблицей `SYNONYMS` в `lib/search/dictionary.ts`.
Новая категория обязана получить там свою запись, иначе находится только по
подписи. Подробности и чек-лист — [SEARCH.md](SEARCH.md).
...
Не коммитить секреты. `local/SECRETS.local.md` содержит локальные/private сведения и
