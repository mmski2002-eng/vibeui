...
- Features 001 / Spotlight Features — **первая вторая категория**: тёмная
  графитовая секция с иерархией «одна главная возможность + три второстепенные».
  В репозитории, на production ещё не задеплоена.
...
11. задеплоить Features 001 и проверить установку по HTTPS в чистый проект;
12. дальше — `pricing-001`.

Текущая точка: шаг 11. Features 001 лежит в рабочем дереве и проверен локально;
production отдаёт сборку без него. `meta.featured` у features-001 не выставлен
намеренно: главная берёт первые три featured-блока, и это три hero.
