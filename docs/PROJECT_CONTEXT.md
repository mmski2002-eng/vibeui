...
- главная: первый экран показывает продукт — слева обещание и два CTA,
  справа композиция из настоящих миниатюр блоков (`components/page-composition.tsx`);
- секция «Собери страницу из блоков» объясняет, что типы секций стыкуются;
- featured-секция объясняет, что три hero — это разные дизайн-направления;
- каталог: фильтр категорий (`components/catalog-filter.tsx`) — клиент
  переключает только `data-catalog-filter`, карточки остаются серверными,
  правила фильтра выводятся из `CATEGORIES`, страница остаётся статической;
- страница блока: панель «Использовать с AI» с primary-кнопкой Copy for AI
  стоит сразу под превью, установка вручную — внутри неё, исходник ниже
  как второстепенное.

Важно: витрина нигде не копирует блоки. И композиция, и цепочка, и карточки
рендерят те же компоненты из `registry/blocks/`. Обернуть миниатюру в ссылку
нельзя — внутри блоков есть свои `<a>`, вложенные ссылки дают ошибку
гидратации; кликабельность даёт `BlockCard` растянутым псевдоэлементом.
